...
| A trace (`traces/<match>.json` or the file of `play --trace`) | **Match**: the box score first — what each side added up to, the most valuable creature, damage dealt per creature as a chart, one row per creature (damage dealt and taken, healing, kills, casts, fizzles, crits, conditions, energy, health left, the spell it declared most) and one row per spell per side (casts, fizzles, resolve rate, damage, healing, stuns, bleeds, buffs) — then round by round, every event with both teams after it: health bars, energy, defense, initiative, conditions as chips, known spells, intents, the timeline with revealed and resolved actions, outcomes, fizzles, and crits. Step with the buttons or the arrow keys, jump to a round. |
| `evaluation.json` (from `evaluate` or `benchmark`) | **Evaluation**: both agents with win rates and paired intervals, scores, remaining health, spell entropy, fizzle and crit rates, **which spells the winning side was holding and what those spells did** (casts, resolve rate, damage, healing, stuns, bleeds, buffs), a damage-by-spell and a landed-against-fizzled chart, what every number means, and every seed pair. Two agents of the same spec are called out: the mirrored pass then replays the same matches, so the even split is arithmetic. |
...
| Two artifacts of the same kind, through the Compare selects | **Comparison**: the metrics side by side with deltas, and the run stamp diff (what moved: content, engine, rules, schema, agents, seeds). For two evaluations, also the spell-by-spell delta — the share of sides that declared a spell and won, and the casts, resolve rate and damage before and after, which are the columns a cost change actually moves. Keyed by spell name, so a spell cut as a new version lines up with the one it replaced. |
