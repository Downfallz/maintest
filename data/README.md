...
- `Tiers/*.json`: the packages one evolution pick buys (`docs/domain/tier-evolution-plan.md`). **Derived, not
  hand-authored**: `scripts/build-tiers.py` writes them from the tree, so edit the tree and re-run it. Nothing
  loads them yet -- the schema and the data builder come next, and until they do these files are content no
  validation covers.
