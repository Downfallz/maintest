...
namespace DA.Core.Game.AI.MonteCarlo
{
    public class State
    {
        public State() { }
        ...
        private List<CharacterActionChoice> GetAvailablePosition(IBattleEngine be)
        {
            ...
            foreach (var s in charToPlay.CastableSpells)
            {
                Team teamTarget = null;
                if (s.SpellType == Domain.Base.Talents.Enum.SpellType.Defensive)
                {
                ...
                else if (s.SpellType == Domain.Base.Talents.Enum.SpellType.Offensive)
                {
                    if (charToPlay.TeamNumber == 1)
                    {
                        teamTarget = Board.TeamTwo;
        ...
        private void Setup(IBattleEngine be, Battle battle)
        {
            ...
            be.ChooseSpellToUnlock(battle, Domain.Base.Teams.Enum.TeamIndicator.One, t1Choice);
            be.ChooseSpellToUnlock(battle, Domain.Base.Teams.Enum.TeamIndicator.Two, t2Choice);
            be.ChooseSpeed(battle, Domain.Base.Teams.Enum.TeamIndicator.One, t1SpeedChoice);
            be.ChooseSpeed(battle, Domain.Base.Teams.Enum.TeamIndicator.Two, t2SpeedChoice);
        }
