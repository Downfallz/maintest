...
namespace DA.AI.CharAction
{
    ...
    public class IntelligentCharacterActionChooser : ICharacterActionChooser
    {
        ...
        public IntelligentCharacterActionChooser(IBattleEngine simulator, IBattleScorer battleScorer)
        {
        ...
        public CharacterActionChoice GetCharActionChoice(Battle battle, Character charToPlay, List<Character> aliveCharacters, List<Character> aliveEnemies)
        {
            ...
            BasicBattleScorer battleScorer = new BasicBattleScorer();
            BestPick bestPick = new BestPick();
            bestPick.BestScore = battleScorer.GetBattleScore(battle);
            ...
            foreach (var s in possibleSpell)
            {
                if (s.SpellType == SpellType.Defensive)
                {
                    ...
                    if (spellTargetCount >= possibleTargetsCount)
                    {
                        ...
                        BestScore(battle, charToPlay, s, targets, bestPick);
                    }
                    ...
                    {
                        BestScore(battle, charToPlay, s, new List<Guid>(), bestPick);
                    }
        ...
        private void BestScore(Battle battle, Character charToPlay, Spell s, List<Guid> targets, BestPick pick)
        {
