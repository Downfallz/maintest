...
namespace DA.Core.Game
{
    public class BattleEngine : IBattleEngine
    {
        ...
        public void ChooseSpellToUnlock(Battle battle, TeamIndicator ti, List<SpellUnlockChoice> choices)
        {
            ...
            if (_roundService.ChooseSpellToUnlock(battle, ti, choices))
            {
        ...
        {
            if (_roundService.ChooseSpeed(battle, ti, choices))
            {
                _roundService.ResolveCharacterOrder(battle);
                var c = _roundService.GetCurrentCharacterId(battle.CurrentRound);
