    ...
    public class CslUiPlayerHandler : BasePlayerHandler
    {
        ...
        public override void EvaluateCharacterToPlay(object sender, CharacterTurnInitializedEventArgs e)
        {
            ...
            if (characterToPlay != null)
            {
                ...
                var spellCount = characterToPlay.CharacterTalentStats.UnlockedSpells.Count;
                ...
                {
                    var tal = characterToPlay.CharacterTalentStats.UnlockedSpells[i];
                ...
                var spellToPlay = characterToPlay.CharacterTalentStats.UnlockedSpells[numberPicked];

                var targets = new List<Guid>();
                if (spellToPlay.SpellType == SpellType.Defensive)
                {
                    var possibleTargetsCount = MyAliveCharacters.Count;
                    var spellTargetCount = spellToPlay.NbTargets;

                    Console.WriteLine($"Spell can target {spellTargetCount} out of {possibleTargetsCount} possible targets.");
                    for (var i = 0; i < possibleTargetsCount; i++)
                    {
                        Console.WriteLine($"{i} - {MyAliveCharacters[i]}");
                    }

                    for (var i=0; i<spellTargetCount; i++)
                    {
                ...
                {
                    var possibleTargetsCount = MyEnemies.Count;
                    var spellTargetCount = spellToPlay.NbTargets;
        ...
        public override void SpellUnlock(object sender, EventArgs e)
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                for (var j = 0; j < MyAliveCharacters.Count; j++)
                {
                ...
                var c = MyAliveCharacters[numberPicked];
                Console.ForegroundColor = ConsoleColor.Gray;
                ...
                for (var j = 0; j < possibleListCount; j++)
                {
                    Console.ForegroundColor = ConsoleColor.Gray;
