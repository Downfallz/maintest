...
dotnet run --project src/DownfallArena.Cli -- table --rules <file> --p2 greedy   # two people at one screen, or one against a bot (ADR 0054); --handover N starts as bots and hands over at round N; --bind <address> serves a phone on the same network instead of this machine only; the session is recorded into runs/playtest/<id>/ (--record names another root, --who <initials> stamps who played), and /session/<id> shows it in the viewer once the match is over
