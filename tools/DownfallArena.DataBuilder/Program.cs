...
try
{
    ...
    await Console.Out.WriteLineAsync($"Built {schema.Spells.Count} spells, {schema.Creatures.Count} creatures, {schema.TalentTrees.Count} talent trees, {schema.Tiers?.Count ?? 0} tiers from '{dataDirectory}'.");
