...
public sealed class TableSessionTests : IDisposable
{
    ...
    {
        var services = Built();
        var session = await TableSession.StartAsync(services, RuleSet.Create(2, 2, 1, 6, 2.0), seed: 7, Bot(services), Bot(services), TestContext.Current.CancellationToken);
    ...
    public async Task A_seat_handed_to_someone_else_mid_match_keeps_the_match_running()
    {
        ...
        var session = await TableSession.StartAsync(services, RuleSet.Create(2, 2, 1, 6, 2.0), seed: 7, first, Bot(services), TestContext.Current.CancellationToken);
