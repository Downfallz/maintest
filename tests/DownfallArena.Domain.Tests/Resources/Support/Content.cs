...
internal static class Content
{
    ...
    public static Spell Spell(string id, TargetingSpec targeting, int cost = 0, double criticalChance = 0, params Effect[] effects) =>
        Build(id, targeting, cost, criticalChance, effects);
    ...
    public static Spell SpellWithCasterEffects(string id, Effect[] effects, params Effect[] casterEffects) =>
        Build(id, TargetingSpec.SingleTarget(TargetOrigin.Enemy), cost: 0, criticalChance: 0, effects, casterEffects);

    private static Spell Build(string id, TargetingSpec targeting, int cost, double criticalChance, Effect[] effects, Effect[]? casterEffects = null) =>
        Domain.Resources.Spell.Create(
            SpellId.Parse(id),
            "Strike",
            SpellType.Offensive,
            CreatureClass.Creature,
            new SpellStats(Energy.Of(cost), CriticalChance.Of(criticalChance)),
            targeting,
