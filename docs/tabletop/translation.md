...
2026-09-25). Phase 1 of [plan.md](plan.md).
...
| The Round cap ends the Match on total remaining Health | `completedRound >= rules.RoundCap` (`WinCondition.cs:23-26`) | 2 sums of 3 numbers, once, at the end | **keep as is** | Nothing. The cap's *value* is a `RuleSet` parameter, so a shorter table Match costs no fidelity; the number is the maintainer's, set at 10 to 15 Rounds for a 15 to 30 minute Match ([plan.md](plan.md), ADR 0068), with a table cap of 20, not a rule change. |
...
- **The 30-Round cap.** It is a `RuleSet` parameter (`RuleSet.cs:30,42`), and the engine plays any value
  ...
  needs no ADR. The number is the maintainer's, set at 10 to 15 Rounds for a 15 to 30 minute Match
