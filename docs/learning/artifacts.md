...
| `code` | The numeric form of the chosen action: `kind`, `actingSlot`, `spellIndex`, `speed`, `targetMask`. On an `Evolve` action `spellIndex` carries the **tier** index of the package bought (ADR 0056), which is why that action's schema version moved. |
