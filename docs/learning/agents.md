...
- **Evolution**: for each unlockable spell, its value as if the creature knew it and could afford it (the
  ...
  can be unlocked. This is the one decision the `initiative` weight touches: it is not part of a resolution,
  so it is absent from the table above.
...
| initiative | 0.5 | Half a point per point of base initiative an unlock buys. Initiative only reorders the timeline, so it is priced like defense: real, indirect, and worth less than the hit it may let you land first. A first guess, and the weight `search-weights` has the least evidence about. |
