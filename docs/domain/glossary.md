...
| Defense buff ceiling | The most a Creature's Defense buffs add to its total Defense, permanent and timed together: 10 (ADR 0076). A buff past it is still held and counts down; it adds nothing until others expire. | decided |
...
| Stacking policy | What applying a lasting Effect does when the Creature already carries it: `Stack` (add another), `Refresh` (restart the duration), `Ignore`. Every kind stacks unless the content says otherwise; Stun is the one that is ignored, because a second stun on a stunned Creature would only extend the first, and the rules bound how long a Creature is stunned (ADR 0072; it refreshed under ADR 0041). | decided |
| Stun immunity | The Round after a Creature's Stun ends, during which a Stun cast on it is ignored (ADR 0072). A state of the Creature, not a Condition: no Spell carries it and nothing is charged for it. It starts at the Cleanup that ends the Stun and ends at the next one. | decided |
