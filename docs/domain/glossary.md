...
| Tier | A named package of Spells one evolution pick buys, with a level (1 opens a family, 3 closes one), the Tiers it requires, and one initiative bonus. Prerequisites are the only rule that decides which Tiers a Creature may buy, one an Evolution opportunity (ADR 0066), and a Tier must require one exactly a level below it (ADR 0056). | decided |
