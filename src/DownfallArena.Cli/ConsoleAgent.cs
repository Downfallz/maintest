...
internal sealed class ConsoleAgent(TextReader input, TextWriter output) : IPlayerAgent
{
    public EvolutionDecision DecideEvolution(PlayerBoardState board, EvolutionOptions options)
    {
        ...
        var choices = options.Creatures
            .SelectMany(creature => creature.UnlockableSpells.Select(spell => new EvolutionChoice(creature.Creature, spell)))
            .ToList();
        var picked = Pick("Unlock", choices.Select(choice => $"creature {choice.Creature}: {choice.Spell.Value}").ToList(), allowNone: "pass");
    ...
    public SpellId DecideIntent(PlayerBoardState board, IntentOption intentOption)
    {
        ...
        ShowBoard(board);
        var index = Pick($"Intent of creature {intentOption.Creature}", intentOption.CastableSpells.Select(spell => spell.Value).ToList());
    ...
    public IReadOnlyList<CreatureId> DecideTargets(PlayerBoardState board, TargetOptions options)
    {
        ...
        {
            var remaining = legal.Candidates.Where(candidate => !targets.Contains(candidate)).ToList();
            var allowNone = targets.Count >= legal.MinTargets ? "done" : null;
            var index = Pick($"Target {targets.Count + 1} of {options.Spell.Value} by creature {options.Actor}", remaining.Select(candidate => $"creature {candidate}").ToList(), allowNone);
            if (index is null)
            {
                break;
            }
    ...
    /// </summary>
    private int? Pick(string title, IReadOnlyList<string> entries, string? allowNone = null)
    {
        while (true)
        {
            ...
            if (line is null)
            {
                throw new InvalidOperationException("The input ended before the match did.");
            }

            if (int.TryParse(line, NumberStyles.Integer, CultureInfo.InvariantCulture, out var number))
            {
                if (number == 0 && allowNone is not null)
                {
                    return null;
                }
