...
public sealed class FeatureSchema
{
    ...
    public static IReadOnlyList<string> ConditionKinds { get; } = ["Bleed", "Regeneration", "EnergyRegeneration", "Stun", "DefenseBuff", "DefenseDebuff", "InitiativeBuff", "InitiativeDebuff"];
    ...
    private readonly Dictionary<TierId, int> _tierIndexes;

    private FeatureSchema(int teamSize, int roundCap, IReadOnlyList<SpellId> spells, IReadOnlyList<TierId> tiers)
    {
        ...
        _tierIndexes = tiers.Select((tier, index) => (tier, index)).ToDictionary(pair => pair.tier, pair => pair.index);
        FeatureNames = [.. GlobalFeatures, .. CreatureBlocks(teamSize, spells, tiers)];
    ...
    /// <summary>
    /// Every package of the content, sorted by id, in feature order. It replaced the talent-node block when a
    /// pick started buying a package: a node was only ever a guess at what a creature had bought, read back
    /// from its spells, and the purchase is now recorded (ADR 0056).
    /// </summary>
    public IReadOnlyList<TierId> Tiers { get; }

    public IReadOnlyList<string> FeatureNames { get; }

    public int Length => FeatureNames.Count;

    public int CreatureLength => CreatureFeatures.Count + (2 * ConditionKinds.Count) + Spells.Count + Tiers.Count;
    ...
    public static FeatureSchema Build(IGameResources resources, RuleSet ruleSet)
    {
        ...
        var tiers = resources.Tiers.Select(tier => tier.Id).OrderBy(tier => tier.Value, StringComparer.Ordinal).ToList();
        return new FeatureSchema(ruleSet.TeamSize, ruleSet.RoundCap, spells, tiers);
    }
    ...
    /// <summary>The index of a package among the content's packages, or -1 when it is not in the content.</summary>
    public int TierIndex(TierId tier) => _tierIndexes.GetValueOrDefault(tier, -1);
    ...
    private static IEnumerable<string> CreatureBlocks(int teamSize, IReadOnlyList<SpellId> spells, IReadOnlyList<TierId> tiers)
    {
        foreach (var side in new[] { "own", "enemy" })
        {
            for (var slot = 0; slot < teamSize; slot++)
            {
                foreach (var name in CreatureBlock(spells, tiers))
                {
    ...
    private static IEnumerable<string> CreatureBlock(IReadOnlyList<SpellId> spells, IReadOnlyList<TierId> tiers)
    {
